...
/*   Updated: 2024/06/24 14:54:19 by angomes-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void move_keyhook(mlx_key_data_t keydatam, void *param) {
  ...
  if (keydatam.key == MLX_KEY_W)
    handle_player_movement(game->player, UP);
  if (keydatam.key == MLX_KEY_A)
    handle_player_movement(game->player, LEFT);
  if (keydatam.key == MLX_KEY_D)
    handle_player_movement(game->player, RIGHT);
  if (keydatam.key == MLX_KEY_S)
    handle_player_movement(game->player, DOWN);
}
